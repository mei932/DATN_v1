...
// })
// export class CartComponent implements OnInit {
//   @Output() cartUpdated = new EventEmitter<number>();  // Phát sự kiện khi giỏ hàng thay đổi
//   count: number = 0;
//   cart: any[] = [];
//   cartMap: any[] = [];
//   totalPrice: number = 0;
...
//   ngOnInit(): void {
//     this.cart = sessionStorage.getItem("samanhua-shop-cart")
//       ? JSON.parse(sessionStorage.getItem("samanhua-shop-cart")!)
//       : [];
//     from(this.cart)
//       .pipe(
//         mergeMap((cartEntity: any) =>
//           this.bookService.getBookByOption(cartEntity).pipe(
//             map((book: any) => {
//               let quantity = Math.min(book.book.options[0]?.quantity, cartEntity.quantity);
//               cartEntity.quantity = quantity;
//               book.quantity = quantity;
...
//       )
//       .subscribe((books: any[]) => {
//         this.cart = this.cart.filter((cartEntity) => cartEntity.quantity !== 0);
//         this.count = this.cart.length;
//         sessionStorage.setItem("samanhua-shop-cart", JSON.stringify(this.cart));
//         this.cartMap = books
//           .filter((book) => book.quantity !== 0)
//           .sort((a: any, b: any) => b.time - a.time);
//         this.updateTotalPrice();
//         this.cartUpdated.emit(this.count); // Phát sự kiện cập nhật giỏ hàng
//       });
//   }
...
//     });
//     this.updateTotalPrice();
//     this.cartUpdated.emit(this.count);  // Phát sự kiện khi thay đổi số lượng giỏ hàng
//   }
...
//       title: `Đã xoá sách khỏi giỏ hàng!`,
//     });
//     this.cartUpdated.emit(this.count);  // Phát sự kiện khi sản phẩm bị xoá khỏi giỏ hàng

//   }

//   updateTotalPrice() {
//     this.totalPrice = this.cartMap.reduce(
